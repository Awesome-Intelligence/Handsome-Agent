...
class DashscopeProvider(BaseProvider):
    ...
    def __init__(self, config: ProviderConfig):
        ...
        self.base_url = (
            ...
            or DEFAULT_LLM_BASE_URLS.get("dashscope")
        )
        ...
        **kwargs
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.error(f"Dashscope API error - status: {response.status_code}")
            ...
            self.logger.error(f"Dashscope request failed - {e}")
            raise Exception(f"Failed to call Dashscope API: {e}")
    ...
    async def generate_stream(
        ...
        **kwargs
        ...
        """生成流式响应"""
        start_time = time.time()
        self.logger.info(f"Dashscope streaming request started - model: {self.config.model}")
