...
class LLMClient:
        ...
        **kwargs
        ...
        if self._context_manager:
            try:
                ...
                # 降级：直接调用
                response = await self._provider.generate(user_message)
                return response.content if hasattr(response, 'content') else str(response)
    ...
    async def auxiliary_call(
        ...
        **kwargs
        ...
        self._llm_logger.info(f"Auxiliary call: task={task.value}, model={effective_model}")
    ...
    async def title_call(self, prompt: str, **kwargs) -> str:
        """标题生成调用"""
        return await self.auxiliary_call(
            task=LLMTaskType.TITLE_GENERATION,
            prompt=prompt,
            **kwargs
        )
    ...
    async def memory_call(self, prompt: str, **kwargs) -> str:
        """记忆摘要调用"""
        return await self.auxiliary_call(
            task=LLMTaskType.MEMORY_SUMMARY,
            prompt=prompt,
            **kwargs
        )
    ...
    def invalidate_cache(self) -> None:
        ...
        self._cache_version += 1
        self._logger.debug(f"System prompt cache invalidated (version={self._cache_version})")
