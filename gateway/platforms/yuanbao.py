...
def _check_yuanbao_runtime() -> bool:
    return WEBSOCKETS_AVAILABLE


def _build_yuanbao_adapter(config):
    """Construct a YuanbaoAdapter from a PlatformConfig."""
    return YuanbaoAdapter(config)


def _yuanbao_apply_yaml_config(yaml_cfg: dict, env_overrides: dict):
    """Extract Yuanbao-specific ``extra`` fields from config.yaml."""
    platforms_cfg = (yaml_cfg.get("gateway") or {}).get("platforms") or yaml_cfg.get(
        "platforms"
    ) or {}
    yb_cfg = platforms_cfg.get("yuanbao") or {}
    extra = dict(yb_cfg.get("extra") or yb_cfg)
    import os

    for k_env, k_cfg in (
        ("YUANBAO_APP_ID", "app_id"),
        ("YUANBAO_APP_SECRET", "app_secret"),
        ("YUANBAO_BOT_ID", "bot_id"),
        ("YUANBAO_WS_URL", "ws_url"),
        ("YUANBAO_API_DOMAIN", "api_domain"),
    ):
...
def register() -> None:
    """Register the Yuanbao (Tencent) platform with the gateway registry."""
    from gateway.platforms.platform_registry import PlatformEntry, platform_registry

    platform_registry.register(
        PlatformEntry(
            name="yuanbao",
            label="Yuanbao (Tencent)",
            adapter_factory=_build_yuanbao_adapter,
            check_fn=_check_yuanbao_runtime,
            required_env=["YUANBAO_APP_ID", "YUANBAO_APP_SECRET"],
            install_hint="pip install websockets httpx",
            setup_fn=None,
            apply_yaml_config_fn=_yuanbao_apply_yaml_config,
