# 🚪 Access - gateway/platforms/email/adapter.py
"""
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._imap_port = _stub.env_int("EMAIL_IMAP_PORT", 993)
        self._smtp_host = (
            os.getenv("EMAIL_SMTP_HOST", "") or extra.get("smtp_host", "")
        ).strip()
        self._smtp_port = _stub.env_int("EMAIL_SMTP_PORT", 587)
        self._poll_interval = _stub.env_int("EMAIL_POLL_INTERVAL", 15)
        ...
        elif _stub.env_bool("EMAIL_TRUST_FROM_HEADER", False):
...
def _is_connected(config) -> bool:
    ...
    if extra.get("address"):
        return True
    return bool((_stub.get_env_value_gw("EMAIL_ADDRESS") or "").strip())
