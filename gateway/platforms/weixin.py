...
# hermes_constants stubbed below
...
class WeixinAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dm_policy = (
            str(extra.get("dm_policy") or os.getenv("WEIXIN_DM_POLICY", "pairing"))
            .strip()
            .lower()
        )
        self._group_policy = (
...
def _build_weixin_adapter(config):
    """Construct a WeixinAdapter from a PlatformConfig."""
    return WeixinAdapter(config)


def _weixin_apply_yaml_config(yaml_cfg: dict, env_overrides: dict):
    """Extract Weixin-specific ``extra`` fields from config.yaml."""
    platforms_cfg = (yaml_cfg.get("gateway") or {}).get("platforms") or yaml_cfg.get(
        "platforms"
    ) or {}
    wx_cfg = platforms_cfg.get("weixin") or {}
    extra = dict(wx_cfg.get("extra") or wx_cfg)
    import os
...
def register() -> None:
    """Register the Weixin (personal-account) platform with the gateway registry."""
    ...
    platform_registry.register(
        PlatformEntry(
            name="weixin",
            label="Weixin / WeChat",
            adapter_factory=_build_weixin_adapter,
            check_fn=check_weixin_requirements,
            required_env=["WEIXIN_ACCOUNT_ID", "WEIXIN_TOKEN"],
            install_hint="pip install aiohttp pycryptodome  "
            "(follow the hermes/agentz weixin pairing guide first)",
            setup_fn=None,
            apply_yaml_config_fn=_weixin_apply_yaml_config,
