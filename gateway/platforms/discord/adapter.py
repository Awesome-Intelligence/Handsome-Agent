...
_DISCORD_NONCONVERSATIONAL_HISTORY_MESSAGE_PATTERNS = (
    ...
    re.compile(
        r"^\s*(?:✅|❌)\s+Hermes update\s+" r"(?:finished|failed|timed out)[\s\S]*$",
        re.IGNORECASE,
    ),
    re.compile(
...
def check_discord_requirements() -> bool:
    ...
    if DISCORD_AVAILABLE:
        return True
    try:
        from gateway.platforms._hermes_stubs import ensure as _lazy_ensure
...
class VoiceReceiver:
    ...
    def pcm_to_wav(
        ...
        with tempfile.NamedTemporaryFile(suffix=".pcm", delete=False) as f:
            ...
            pcm_path = f.name
        try:
            from gateway.platforms._hermes_stubs import windows_hide_flags
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    """
    try:
        from gateway.platforms._hermes_stubs import read_raw_config
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._voice_sources: Dict[int, Dict[str, Any]] = (
            {}
        )  # guild_id -> linked text channel source metadata
        self._voice_timeout_tasks: Dict[int, asyncio.Task] = (
            {}
        )  # guild_id -> timeout task
        # Phase 2: voice listening
        self._voice_receivers: Dict[int, VoiceReceiver] = (
            {}
            ...
            async def on_message(message: DiscordMessage):
                ...
                if getattr(message.author, "bot", False):
                    ...
                    if not _is_dm:
                        ...
                        _parent_id = adapter_self._get_parent_channel_id(
                            message.channel
                        )
                        if _parent_id:
                ...
                if not isinstance(message.channel, discord.DMChannel) and (
                    ...
                    if (
                        ...
                        _channel_keys = adapter_self._discord_channel_keys(
                            message, _parent_id
                        )
                        if "*" not in _free_channels and not (
                            _channel_keys & _free_channels
                        ):
                            return
    ...
    async def play_ack_in_voice(
        ...
        try:
            from gateway.platforms._hermes_stubs import text_to_speech_tool
    ...
    async def _process_voice_input(self, guild_id: int, user_id: int, pcm_data: bytes):
        ...
        from gateway.platforms._hermes_stubs import is_whisper_hallucination
        ...
        try:
            ...
            from gateway.platforms._hermes_stubs import transcribe_audio
    ...
    def _register_slash_commands(self) -> None:
        ...
        dropped_over_cap = 0
        try:
            from gateway.platforms._hermes_stubs import (
                COMMAND_REGISTRY,
        ...
        try:
            from gateway.platforms._hermes_stubs import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from gateway.platforms._hermes_stubs import discord_skill_commands_by_category
    ...
    async def _fetch_channel_context(
        ...
        try:
            ...
            def _keep(msg) -> Optional[str]:
                ...
                if str(
                    getattr(msg, "id", "")
                ) in self._nonconversational_messages or _looks_like_nonconversational_history_message(
                    content
                ):
                    return None
    ...
    async def send_model_picker(
        ...
        try:
            ...
            try:
                from gateway.platforms._hermes_stubs import get_label
...
def _define_discord_view_classes() -> None:
    ...
    class ExecApprovalView(discord.ui.View):
        ...
        async def _resolve(
            ...
            try:
                from gateway.platforms._hermes_stubs import resolve_gateway_approval
    ...
    class ModelPickerView(discord.ui.View):
        ...
        async def _expensive_warning_for(self, model_id: str):
            try:
                from gateway.platforms._hermes_stubs import expensive_model_warning
    ...
    class ClarifyChoiceView(discord.ui.View):
        ...
        async def _resolve_choice(
            ...
            try:
                from gateway.platforms._hermes_stubs import _entries as _clarify_entries  # type: ignore
            ...
            try:
                from gateway.platforms._hermes_stubs import resolve_gateway_clarify
        ...
        async def _on_other(self, interaction: "discord.Interaction") -> None:
            ...
            try:
                from gateway.platforms._hermes_stubs import mark_awaiting_text
...
def _is_connected(config) -> bool:
    ...
    """
    import gateway.platforms._hermes_stubs as gateway_mod
