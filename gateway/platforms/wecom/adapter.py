...
class WeComAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dm_policy = (
            str(extra.get("dm_policy") or os.getenv("WECOM_DM_POLICY", "pairing"))
            .strip()
            .lower()
        )
        ...
        self._group_policy = (
            str(extra.get("group_policy") or os.getenv("WECOM_GROUP_POLICY", "pairing"))
            .strip()
            .lower()
        )
    ...
    async def _cache_media(
        ...
        if "base64" in media and media.get("base64"):
            ...
            return (
                cache_document_from_bytes(raw, filename),
    ...
    async def _send_media_source(
        ...
        return SendResult(
            ...
            raw_response={
                ...
                "downgrade": (
                    downgrade_result.raw_response if downgrade_result else None
                ),
                "downgrade_error": (
                    downgrade_result.error
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        ...
        return {
            "name": chat_id,
            "type": (
                "group" if chat_id and chat_id.lower().startswith("group") else "dm"
            ),
        }
...
def _build_callback_adapter(config):
    ...
    from gateway.platforms.wecom.callback_adapter import WecomCallbackAdapter

    return WecomCallbackAdapter(config)


def register() -> None:
    """Register the WeCom platforms with the Agent-Z gateway registry."""
    from gateway.platforms.platform_registry import PlatformEntry, platform_registry
    from gateway.platforms.wecom.callback_adapter import (
    ...
    platform_registry.register(
        PlatformEntry(
            ...
            install_hint="pip install aiohttp httpx defusedxml",
