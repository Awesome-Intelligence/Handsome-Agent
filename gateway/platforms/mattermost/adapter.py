# 🚪 Access - gateway/platforms/mattermost/adapter.py
            ...
            async with session.post(
                ...
                **_req_kw,
                ...
                if resp.status not in {200, 201}:
                    ...
                    return {
                        "error": (
                            f"Mattermost API error ({resp.status}): " f"{body[:400]}"
                        )
                    }
                data = await resp.json()
            return {
...
def interactive_setup() -> None:
    ...
    removes.
    """
    _stub.get_env_value, _stub.save_env_value
    _stub.prompt, _stub.prompt_yes_no, _stub.print_header, _stub.print_info, _stub.print_success

    _stub.print_header("Mattermost")
    existing = _stub.get_env_value("MATTERMOST_TOKEN")
    if existing:
        _stub.print_info("Mattermost: already configured")
        if not _stub.prompt_yes_no("Reconfigure Mattermost?", False):
            return

    _stub.print_info("Works with any self-hosted Mattermost instance.")
    _stub.print_info(
        "   1. In Mattermost: Integrations → Bot Accounts → Add Bot Account"
    )
    _stub.print_info("   2. Copy the bot token")
    print()
    mm_url = _stub.prompt("Mattermost server URL (e.g. https://mm.example.com)")
    if mm_url:
        _stub.save_env_value("MATTERMOST_URL", mm_url.rstrip("/"))
    token = _stub.prompt("Bot token", password=True)
    if not token:
        return
    _stub.save_env_value("MATTERMOST_TOKEN", token)
    _stub.print_success("Mattermost token saved")

    print()
    _stub.print_info("🔒 Security: Restrict who can use your bot")
    _stub.print_info("   To find your user ID: click your avatar → Profile")
    _stub.print_info("   or use the API: GET /api/v4/users/me")
    print()
    allowed_users = _stub.prompt(
        "Allowed user IDs (comma-separated, leave empty for open access)"
    )
    if allowed_users:
        _stub.save_env_value("MATTERMOST_ALLOWED_USERS", allowed_users.replace(" ", ""))
        _stub.print_success("Mattermost allowlist configured")
    else:
        _stub.print_info(
    ...
    home_channel = _stub.prompt(
        "Home channel ID (leave empty to set later with /set-home)"
    )
    if home_channel:
        _stub.save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    _stub.print_info("   Open config in your editor:  hermes config edit")
...
def _is_connected(config) -> bool:
    ...
    """
    return bool(
        (_stub.get_env_value_gw("MATTERMOST_TOKEN") or "").strip()
        and (_stub.get_env_value_gw("MATTERMOST_URL") or "").strip()
    )
