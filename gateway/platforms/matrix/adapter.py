...
def _resolve_matrix_bang_command(name: str) -> str | None:
    ...
    *name* only by underscore→hyphen normalization, e.g. ``reload_skills`` →
    ...
    try:
        from gateway.platforms._hermes_stubs import is_gateway_known_command
    ...
    try:
        from gateway.platforms._hermes_stubs import get_skill_commands
...
from gateway.platforms._hermes_stubs import get_hermes_dir as _get_hermes_dir
...
def check_matrix_requirements() -> bool:
    ...
    try:
        from gateway.platforms._hermes_stubs import feature_missing, ensure_and_bind
...
def _is_connected(config) -> bool:
    ...
    from gateway.platforms._hermes_stubs import get_env_value_gw

    homeserver = extra.get("homeserver") or get_env_value_gw("MATRIX_HOMESERVER") or ""
    token = (
        getattr(config, "token", None)
        or get_env_value_gw("MATRIX_ACCESS_TOKEN")
        or get_env_value_gw("MATRIX_PASSWORD")
        or ""
    )
