...
# ---------------------------------------------------------------------------
# Stubs for Hermes imports not yet available in Agent-Z
# ---------------------------------------------------------------------------


def env_float(key: str, default: float = 0.0) -> float:
    """Parse env var as float, returning default if missing or invalid."""
    import os
...
def env_int(key: str, default: int = 0) -> int:
    """Parse env var as int, returning default if missing or invalid."""
    import os
...
def redirect_target_from_response(response) -> str | None:
    """Stub: return redirect URL from response if present, else None."""
    return None


def to_agent_visible_cache_path(path: str) -> str:
    """Stub: return path unchanged for Agent-Z (no cache path translation)."""
    return path


def redact_sensitive_text(text: str) -> str:
    """Stub: redact sensitive text for logs."""
    return "<redacted>"


def get_tool_emoji(tool_name: str) -> str:
    """Stub: return emoji string for a tool name."""
    return "🔧"


def acquire_scoped_lock(
    scope: str, identity: str, metadata: dict | None = None
) -> tuple[bool, dict | None]:
    """Stub: acquire a scoped lock (always succeeds)."""
    return True, None


def release_scoped_lock(scope: str, identity: str) -> None:
    """Stub: release a scoped lock."""
    pass
...
def write_runtime_status(**kwargs) -> None:
    """Stub: no-op runtime status writer."""
    pass


def check_tts_requirements() -> bool:
    """Stub: TTS requirements check."""
    return True


def text_to_speech_tool(*args, **kwargs):
    """Stub: text-to-speech tool."""
    return None


def mark_awaiting_text(*args, **kwargs) -> None:
    """Stub: mark awaiting text."""
    pass


def should_bypass_active_session(*args, **kwargs) -> bool:
    """Stub: session bypass check."""
    return False


# Stream event stubs (used by render_message_event / format_tool_event)
class MessageChunk:
...
class ToolCallChunk:
    def __init__(self, tool_name: str = "", args: dict | None = None, preview: str = ""):
        self.tool_name = tool_name
        self.args = args or {}
        self.preview = preview


def _get_agent_z_home() -> Path:
...
async def _ssrf_redirect_guard(response):
    ...
    """
    from common.security.url_safety import is_safe_url
...
def get_inbound_media_max_bytes() -> int:
    ...
    """
    try:
        # from hermes_cli.config import load_config as _load_config stubbed
        cfg = _load_config()
    except Exception:
...
def cache_media_bytes(
    ...
    *,
    ...
    """
    # tools.credential_files stubbed
...
class BasePlatformAdapter(ABC):
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        # thread/channel.
        self._authorization_check: Optional[
    ...
    def render_message_event(self, event: Any, sink: Any) -> None:
        ...
        """
        # gateway.stream_events stubbed: MessageChunk, MessageStop, Commentary
    ...
    def format_tool_event(
        ...
        """
        # gateway.stream_events stubbed: ToolCallChunk
        if not isinstance(event, ToolCallChunk):
            return None

        # agent.display stubbed
    ...
    def _write_runtime_status_safe(self, context: str, **kwargs) -> None:
        ...
        """
        try:
            gateway_status_stub = True  # stubbed
    ...
    def _get_ephemeral_system_ttl_default(self) -> int:
        ...
        """
        # Config loading stubbed - always return default
        return 0
    ...
    async def send_clarify(
        ...
        if choices:
            ...
            # from tools.clarify_gateway import mark_awaiting_text stubbed
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            # from hermes_cli.commands import should_bypass_active_session stubbed
        ...
        try:
            ...
            if response:
                ...
                if (
                    ...
                    try:
                        # from tools.tts_tool import text_to_speech_tool, check_tts_requirements stubbed
