...
│                   🏃 执行层 (混合/可选)                      │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐         │
│  │ 💻LocalShell│  │ 🐳Docker/SSH │  │ Computer Use│ (OC)   │
...
| 架构层 | 子层/组件 | 对应目录 | 职责说明 |
|--------|-----------|----------|----------|
| **🚪 接入层** | 📨 MessageRouter, 🔐 AuthHandler | `adapter/` | 多渠道接入、消息路由、鉴权 |
| **🧠 决策层** | 🏷️ IntentClassifier | `core/` | 识别用户意图类型 |
| **🧠 决策层** | 💾 MemoryCache, LRUCache | `core/` | 会话记忆、缓存查询 |
| **🧠 决策层** | 🔀 TaskRouter | `core/` | 任务路由分发 |
| **🧠 决策层** | 🔧 SkillManager | `core/` | 技能发现与执行 |
| **🧠 决策层** | 💬 SessionManager | `core/` | 会话管理、历史记录 |
| **🧠 决策层** | 🤖 AgentLoop, LLM集成 | `brain/` | 核心 Agent 循环推理 |
| **🧠 决策层** | 📝 Summarizer | `brain/memory/` | 对话摘要、上下文压缩 |
| **🧠 决策层** | 📚 KnowledgeBase | `brain/advanced_reasoning/` | 知识库检索 |
| **🧠 决策层** | 🔬 TrajectoryEvaluator, 🛠️ SkillSynthesizer | `brain_curator/` | 异步自我进化 (Curator) |
| **🏃 执行层** | 💻 ShellExecutor | `executor/` | Shell 命令执行 |
| **🏃 执行层** | 🐳 DockerExecutor | `executor/` | Docker 隔离执行 |
| **🛠️ 工具层** | 📦 ToolRegistry, ToolSchema | `tools/` | 工具注册、Schema 定义 |
...
│   │   ├── summarizer.py   # 📝 Summarizer 上下文压缩
...
│   ├── router.py           # 🔀 TaskRouter / 🏷️ IntentClassifier
│   ├── session.py          # 💬 SessionManager 会话管理
│   ├── cache.py           # 💾 MemoryCache / LRUCache
│   ├── layer_logger.py     # 📊 分层日志
│   ├── intent_classifier.py # 🏷️ 意图识别器
│   └── task_middleware.py  # 任务中间件
│
├── shared/                   # 📦 共享模块
│   ├── config.py           # 配置管理
│   ├── logging.py         # 📊 日志配置
...
- **🛠️ HermesToolAdapter** / **🛠️ OpenClawToolAdapter** 处理来源转换
...
- **🛠️ SkillSynthesizer** - 从成功案例提取技能
- **✍️ SkillWriter** - 写入 `~/.skills/` 目录
